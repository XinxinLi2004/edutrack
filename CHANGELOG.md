...
## [v2.3] — 2026-07-17 · 排名对比工具

> 新增独立工具页面，支持两次考试成绩的排名进退步分析。

### 新功能

**排名进退步分析** (`/rank-compare`)

- **数据输入**：两个文本域分别粘贴两次考试的成绩数据（每行 `姓名 成绩`），支持空格/逗号/Tab 分隔，自动跳过表头行
- **排名算法**：按分数降序排名，同分同名次（后续名次顺延），与标准竞赛排名一致
- **对比逻辑**：按姓名匹配两次考试学生 → 计算排名变化（`rank1 - rank2`，正数=进步）→ 分数变化
- **边界处理**：只在一场考试出现的标注为「新增」或「退出」，不计入进退步统计；同名自动去重
- **可视化输出**：
  - 5 张统计卡片（总人数 / 上升 / 下降 / 不变 / 新增退出）
  - 明细对比表（两次分数+排名、分数变化、排名变化、趋势标签）
  - 彩色标签：绿=上升，红=下降，灰=持平，紫=新增，橙=退出
- **排序**：客户端 JS 排序，支持排名变化、分数变化、姓名、第一次排名、第二次排名 5 种维度，点击切换升/降序

### 文件变更

| 文件 | 变更 |
|---|---|
| `templates/rank-compare/index.html` | 新建，工具页面模板（~320 行） |
| `app.py` | 新增 GET/POST 路由 `/rank-compare`，2 个 handler + 2 个静态辅助方法（`_parse_exam_data`、`_compute_ranks`） |
| `templates/partials/header.html` | 侧边栏新增「排名对比」导航项 |

### 技术要点

- 解析逻辑内置表头关键词检测（姓名/名字/score 等），自动跳过 CSV 表头
- `_compute_ranks` 使用状态机式排名算法（prev_score + same_rank_count），O(n log n)
- 排序按钮使用客户端 JS（Array.sort + DOM re-append），无需重新请求服务端
- `do_POST` 新增 try/except 异常保护，避免非法 POST 导致线程崩溃
...
| GET `/rank-compare` → 200（空白表单） | ✅ |
| POST 3 人测试数据 → 200（正确排名+标签） | ✅ |
| POST 10 人中文数据 → 200（服务器存活） | ✅ |
| 5 种排序按钮均正常切换 | ✅ |
| 侧边栏导航高亮正确 | ✅ |
...
| 优先级 | 事项 | 说明 |
|---|---|---|
| 🟢 | 导出功能 | 对比结果暂不支持导出为 CSV/Excel |
| 🟢 | 可视化图表 | 可补充排名变化柱状图（Chart.js） |
